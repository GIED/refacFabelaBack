...
public interface ProductosAlternativosRepository extends JpaRepository<TwProductosAlternativo, Long> {
	...
	@Query(value = "select p from TwProductosAlternativo p where p.nIdProducto=:nIdProducto and p.nEstatus=:nEsatus and tcProductoAlternativo.nEstatus=1 order by tcProductoAlternativo.sNoParte asc ")

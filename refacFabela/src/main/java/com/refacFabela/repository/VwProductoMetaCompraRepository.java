...
public interface VwProductoMetaCompraRepository extends JpaRepository<VwProductoMetaCompra, Long> {
	...
	@Query(value = "SELECT * FROM vw_producto_meta_compra c WHERE c.d_ultima_fecha_compra >= STR_TO_DATE(:fechaInicio, '%Y-%m-%d') AND c.d_ultima_fecha_compra <= STR_TO_DATE(:fechaTermino, '%Y-%m-%d')", nativeQuery = true)
